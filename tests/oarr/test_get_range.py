...
class RangeTest(NDBArrayTest):
  async def asyncSetUp(self):
    ...
    await self.arrays.arr_create('arr', self.size)
    size = await self.arrays.arr_len('arr')
    self.assertEqual(size, self.size)

    for i in range(0, self.size):
      await self.arrays.arr_set('arr', i, {f'k{i}':i})
...
class GetRange(RangeTest):
  ...
  async def test_small(self):
    ...
    values = await self.arrays.arr_get_rng('arr', 0, stop)
  ...
  async def test_start_stop_eq(self):
    values = await self.arrays.arr_get_rng('arr', 0, 0)
    self.assertEqual(len(values), 0)

  
  async def test_rng_one(self):
    values = await self.arrays.arr_get_rng('arr', 0, 1)
