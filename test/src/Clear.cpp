...
TEST_F(NemesisTest, Data)
{
	...
	tc.test({TestData { .request = R"({ "KV_SET":{"keys":{"string1":"billybob"}}})"_json,	.expected = {R"({ "KV_SET_RSP":{ "st":1 } })"_json} }});
	tc.test({TestData { .request = R"({ "KV_SET":{"keys":{"anotherkey":"billybob"}}})"_json,	.expected = {R"({ "KV_SET_RSP":{ "st":1 } })"_json} }});
