...
TEST_F(NemesisTest, New_ExpiryWrongType)
{
  ...
  {
		TestData td {MakeTestData(R"({ "SH_NEW":{ "expiry":"aye" }})"_json)};
		tc.test(td);
...
TEST_F(NemesisTest, New_ExpiryClearNotSet)
{
  ...
  {
		TestData td {MakeTestData(R"({ "SH_NEW":{ "expiry":{"duration":10} }})"_json)};
		tc.test(td);
...
TEST_F(NemesisTest, New_SharedNotSet)
{
  ...
  TestData sesh1 {  .request =  R"({ "SH_NEW":{ }})"_json,
									  .expected = {R"({ "SH_NEW_RSP":{"st":1}})"_json}};

  tc.token = sesh1.token;

  TestData sesh1Check {   .request =  R"({ "SH_INFO":{ }})"_json,
									        .expected = {R"({ "SH_INFO_RSP":{"st":1, "shared":false, "keyCnt":0, "expires":false}})"_json}};
...
TEST_F(NemesisTest, New_ExpirySet)
{
  ...
  TestData sesh1 {  .request =  R"({ "SH_NEW":{ "expiry":{"duration":10, "deleteSession":true} }})"_json,
									  .nResponses = 1,
  ...
  ASSERT_TRUE(sesh1Info.actual[0]["SH_INFO_RSP"].contains("expires"));
  ASSERT_EQ(sesh1Info.actual[0]["SH_INFO_RSP"]["expires"], true);
  ...
  ASSERT_TRUE(sesh1Info.actual[0]["SH_INFO_RSP"]["expiry"].contains("deleteSession"));
...
TEST_F(NemesisTest, New_ExpiryClearOnly)
{
  ...
  TestData sesh1 {  .request =  R"({ "SH_NEW":{ "expiry":{"duration":10, "deleteSession":false} }})"_json,
									  .nResponses = 1,
                    .checkResponses = false};
...
/* TODO waiting for SH_NEW_SHARED
